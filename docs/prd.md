# Figgo — Product Requirements Document (v1.0)

## 1) Summary

Figgo is a Go library and CLI for FIGlet-compatible ASCII art rendering with correct layout handling (kerning/smushing) and a clean, race-safe API.
MVP targets horizontal rendering with FIGfont v2 compliance and ASCII (32–126) only.
...
- It's valid to have FitSmushing with no rules → universal smushing only.
